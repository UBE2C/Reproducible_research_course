...
```{r code to calculate the total number of steps}
...
- the mean of the total number of steps was: `r step_sum_mean`

- the median of the total number of steps was: `r step_sum_median`
...
```{r code to calculate the step means}
...
```{r code to calculate max_steps_mean}
max_steps_mean <- grep(max(step_means), step_means)
```

According to my calculations, the `r max_steps_mean`th 5-minute interval contains on average across all the days the maximum number of steps.
...
```{r code to check the number of NAs}
na_sum <- sum(is.na.data.frame(activity))
```

In total I found `r na_sum` missing values.
...
```{r code to calculate the new total number of steps}
...
```{r code to create a new histogram}
...
```{r code to calculate the new mean and median}
...
- the new mean of the total number of steps was: `r step_sum_mean_imputed`

- the new median of the total number of steps was: `r step_sum_median_imputed`
...
```{r code to create the day factor variable}
...
```{r code to calculate the number if steps on weekdays or weekends}
...
```{r code to create the weekend and weekday line plot}
par(mfrow = c(2, 1))
